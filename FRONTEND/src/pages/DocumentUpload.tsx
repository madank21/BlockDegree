  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        // ─── Verify identity using the backend endpoint ───────────────────
        ...
        try {
          ...
          const updatedDoc = verifyResult;
          setDocuments(prev => prev.map(doc =>
            doc.id === backendDocId
              ? {
                  ...doc,
                  validationStatus: updatedDoc.validation_status,
                  validationErrors: updatedDoc.validation_errors || [],
                }
              : doc
          ));
        } catch (err) {
                      ...
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <div>
                          ...
                          <p className={`text-xs font-medium ${existingDoc.validationStatus === 'valid' ? 'text-green-400' : 'text-red-400'}`}>
                            {existingDoc.validationStatus === 'valid' ? '✓ Name matches profile' : '✗ Name mismatch'}
                          </p>
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Compares extracted name with your profile name</p>
            </div>
          </div>
        </div>
      </div>
