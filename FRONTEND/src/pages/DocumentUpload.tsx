...
//
// FIXES APPLIED:
//   [1] documentsApi.checkFraud removed — endpoint does not exist in backend.
//       Document fraud is assessed by the backend automatically on upload/verify.
//   [2] api.put(...)  → documentsApi.update(id, payload)  (typed, consistent)
//   [3] api.delete(...) → documentsApi.delete(id)         (typed, consistent)
//   [4] Import: removed unused `api` default import; all calls now use named exports.
//
...
// [FIX 4] Removed default `api` import — use named exports only
...
export default function DocumentUpload() {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (Array.isArray(res)) {
        ...
        rawDocs = (res as any).documents || (res as any).data || [];
      }
  ...
  // ── Local YOLO (canvas-based image quality analysis) ─────────────────────────
  const analyzeDocumentWithYOLO = async (
    imageData: string,
    docType: string
  ): Promise<{ valid: boolean; detections: string[]; confidence: number }> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (docType === 'cnic') {
          if (aspectRatio > 1.4 && aspectRatio < 1.8) { detections.push('✓ CNIC Card Detected'); confidence += 10; }
          if (avgBrightness > 100 && avgBrightness < 220) { detections.push('✓ Photo Region Found'); confidence += 5; }
          if (edgeDensity > 0.01) { detections.push('✓ Text Regions Detected'); confidence += 5; }
        } else if (docType === 'marksheet' || docType === 'certificate') {
          if (aspectRatio > 0.6 && aspectRatio < 0.9) { detections.push('✓ Portrait Document Layout'); confidence += 10; }
          else if (aspectRatio > 1.2 && aspectRatio < 1.6) { detections.push('✓ Landscape Document Layout'); confidence += 10; }
          if (edgeDensity > 0.005) { detections.push('✓ Table/Grid Structure Found'); confidence += 5; }
          if (avgColorVariance < 50) { detections.push('✓ Official Document Colors'); confidence += 5; }
        }

        if (canvas.width > 500 && canvas.height > 300) { detections.push('✓ Adequate Resolution'); confidence += 5; }
        else { detections.push('⚠ Low Resolution'); confidence -= 10; }

        if (avgBrightness > 50 && avgBrightness < 230) { detections.push('✓ Good Lighting'); }
        else { detections.push('⚠ Poor Lighting'); confidence -= 5; }

        if (avgColorVariance > 30) { detections.push('✓ Stamps/Seals Detected'); confidence += 5; }

        resolve({ valid: confidence >= 75, detections, confidence: Math.min(confidence, 98) });
      };
      img.src = imageData;
    });
  };
  ...
  const extractDataFromText = (text: string, docType: string): Record<string, string> => {
    ...
    const extractNameFromLine = (line: string): string | null => {
      ...
      if (words.length >= 2 && words.length <= 4) {
        ...
        if (allCaps && !line.match(/PAKISTAN|IDENTITY|CARD|GOVERNMENT|NATIONAL|ISLAMIC|REPUBLIC|GENDER|DATE|EXPIRY|ISSUE|HOLDER|SIGNATURE/i)) {
          return words.join(' ');
        }
      }
      return null;
    ...
    for (const line of lines) {
      if (/name/i.test(line)) {
        ...
        if (name) { extracted['Name'] = name; break; }
      }
    }
    if (!extracted['Name']) {
      for (const line of lines) {
        const name = extractNameFromLine(line);
        if (name && !line.match(/PAKISTAN|IDENTITY|CARD|GOVERNMENT|NATIONAL|ISLAMIC|REPUBLIC|GENDER|DATE|EXPIRY|ISSUE|HOLDER|SIGNATURE/i)) {
          extracted['Name'] = name; break;
        }
      }
    }
    ...
    for (const line of lines) {
      ...
      if (cnicMatch && !extracted['CNIC Number']) extracted['CNIC Number'] = cnicMatch[0].replace(/\s/g, '-');

      const dateMatch = line.match(datePattern);
      if (dateMatch && !extracted['Date of Birth'] && !extracted['Issue Date']) {
        if (/birth|dob/i.test(line))          extracted['Date of Birth'] = dateMatch[0];
        else if (/issue|expiry/i.test(line))  extracted['Issue Date']    = dateMatch[0];
        else                                   extracted['Date']          = dateMatch[0];
      }

      const fatherMatch = line.match(/(?:father|s\/o|d\/o|w\/o)[:\s]*([A-Za-z\s]{3,40})/i);
      if (fatherMatch && !extracted['Father Name']) extracted['Father Name'] = fatherMatch[1].trim();
      ...
      if (regMatch && !extracted['Registration No']) extracted['Registration No'] = regMatch[1];
      ...
      if (boardMatch && !extracted['Board/University']) extracted['Board/University'] = boardMatch[1].trim();

      if (/male/i.test(line) && !extracted['Gender']) {
        extracted['Gender'] = /female/i.test(line) ? 'Female' : 'Male';
      }
    }
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      formData.append('metadata', JSON.stringify({ type: activeUploadType, userId: currentUser.id }));
      ...
      try {
        ...
        // [FIX 2] api.put → documentsApi.update
        await documentsApi.update(backendDocId, {
        ...
        // ── Identity verification (backend endpoint) ─────────────────────
        setValidatingDocId(backendDocId);
        try {
          const verifyResult = await documentsApi.verify(backendDocId);
          setDocuments(prev => prev.map(doc =>
            doc.id === backendDocId
              ? { ...doc, validationStatus: verifyResult.validation_status, validationErrors: verifyResult.validation_errors || [] }
              : doc
          ));
        } catch (err) {
  ...
  const handleValidateDocument = async (docId: string) => {
    ...
    try {
      ...
      setDocuments(prev => prev.map(doc =>
        doc.id === docId
          ? { ...doc, validationStatus: result.validation_status, validationErrors: result.validation_errors || [] }
          : doc
      ));
    } catch (error: any) {
  ...
  const handleRemoveDocument = async (docId: string, docLabel: string) => {
    if (!confirm(`Remove the ${docLabel} document? This cannot be undone.`)) return;
    try {
      // [FIX 3] api.delete → documentsApi.delete
      await documentsApi.delete(docId);
      setDocuments(prev => prev.filter(d => d.id !== docId));
      setUploadedImages(prev => { const s = { ...prev }; delete s[docId]; return s; });
      setOcrResult(prev   => { const s = { ...prev }; delete s[docId]; return s; });
      setYoloResult(prev  => { const s = { ...prev }; delete s[docId]; return s; });
    } catch (err) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">
          Upload identity and academic documents for OCR extraction and quality validation
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {docTypes.map((dt) => {
          ...
          const docImage         = existingDoc
            ? (uploadedImages[existingDoc.id] || existingDoc.fileUrl)
            : null;
          ...
          return (
            <motion.div
              ...
              className={`relative bg-gray-900/50 border rounded-xl p-6 transition-all ${
                existingDoc
                  ? (docYoloResult?.valid !== false ? 'border-green-500/20' : 'border-yellow-500/20')
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                {dt.required && (
                  <span className="text-[10px] px-2 py-0.5 bg-red-500/10 text-red-400 rounded border border-red-500/20">
                    Required
                  </span>
                )}
              </div>
              ...
              {existingDoc ? (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs capitalize">
                            {isProcessingThis && processing?.stage === 'ocr'
                              ? `${ocrProgress}%`
                    ...
                    <div className="flex items-center gap-2 bg-gray-800/30 rounded-lg px-3 py-2">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs capitalize">
                            {existingDoc.yoloStatus || 'processing'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-800/30 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[10px] text-blue-400">
                          {docOcrResult.confidence.toFixed(1)}% confidence
                        </span>
                      </div>
                      <div className="max-h-24 overflow-y-auto">
                        <p className="text-xs text-gray-300 whitespace-pre-wrap font-mono">
                          {docOcrResult.text.substring(0, 500)}
                          {docOcrResult.text.length > 500 ? '…' : ''}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {existingDoc.validationStatus && (
                    <div className={`rounded-lg p-3 ${
                      ...
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <div>
                          <p className="text-[10px] text-gray-400 uppercase">Identity Verification</p>
                          <p className={`text-xs font-medium ${
                            existingDoc.validationStatus === 'valid' ? 'text-green-400' : 'text-red-400'
                          }`}>
                            {existingDoc.validationStatus === 'valid'
                      ...
                      {existingDoc.validationErrors && existingDoc.validationErrors.length > 0 && (
                        <div className="space-y-1">
                          {existingDoc.validationErrors.map((err: string, i: number) => (
                ...
                >
                  {isUploading ? (
                    <><Loader2 className="w-8 h-8 text-blue-400 animate-spin" /><span className="text-sm text-blue-400">Uploading…</span></>
                  ) : (
                    <><Upload className="w-8 h-8 text-gray-500 group-hover:text-blue-400 transition" /><span className="text-sm text-gray-400 group-hover:text-white transition">Click to Upload</span><span className="text-xs text-gray-600">PNG, JPG, JPEG, PDF</span></>
                  )}
                </button>
              )}
            </motion.div>
                ...
                <div className="flex flex-wrap gap-2">
                  {yoloResult[previewDocId].detections.map((det: string, i: number) => (
                    <span key={i} className={`text-xs px-2 py-1 rounded ${
                      det.startsWith('✓') ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'
                    }`}>
                      {det}
                    </span>
                  ))}
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { title: 'Real OCR Extraction',       desc: 'Extracts name, CNIC, and academic data from documents', bg: 'bg-blue-500/10' },
            { title: 'Document Quality Checks',   desc: 'Analyzes layout, quality, and authenticity markers',   bg: 'bg-purple-500/10' },
            { title: 'Identity Verification',     desc: 'Compares extracted name with your profile name',       bg: 'bg-green-500/10' },
          ].map((item, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className={`w-8 h-8 rounded-lg ${item.bg} flex items-center justify-center shrink-0 text-lg`}>✓</div>
              <div>
