...
export default function AuditLogs() {
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      // FIX: the backend's sendPaginated() envelope is
      //   { success, data: [...], pagination: {...} }
      // and api.request()'s envelope-unwrap returns body.data directly, so
      // the result here is already the array itself — NOT { logs: [...] }.
      // Reading `.logs` off of an array is always undefined, so this page
      // rendered an empty list no matter how much data existed.
      const data = await api.request<AuditLog[]>('/audit-logs');
